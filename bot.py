...
Runs inside the daemon — no separate process needed.
"""
...
class SorcererBot:
    def __init__(self, token, chat_id, db_file, log_fn=print):
        ...
        # Injected by daemon so bot can trigger scans
        self.scan_fn  = None
        self.add_fn   = None
    ...
    def handle_start(self, chat_id):
        self.send(
            ...
            "I watch YouTube channels 24/7 and alert you the moment something goes viral in your niche.\n\n"
    ...
    def handle_status(self, chat_id):
        ...
        self.send(
            ...
            f"📡 Watching: {chans} channel(s)\n"
            f"🔍 Last scan: {last_str}\n"
            f"📊 Total scans: {scans}\n"
            f"🔥 Total alerts fired: {total}\n\n"
            f"Next scan: every 2 hours automatically",
            chat_id
        )
    ...
    def start_polling(self):
        """Run in a background thread — polls Telegram for new messages."""
        self.running = True
