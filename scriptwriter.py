...
def format_script_terminal(script):
      ...
      if not script or script.get("_error"):
                return f"  ⚠  Script error: {(script or {}).get('_error', 'unknown')}"
            lines = []
    lines.append(f"\n  ╔{'═' * 60}╗")
    lines.append(f"  ║ 📄 SCRIPT: {script.get('title', 'Untitled')[:48]:<48} ║")
    lines.append(f"  ╠{'═' * 60}╣")
    funny = script.get("funniest_line", "")
    if funny:
              lines.append(f"  ║ 😂 {funny[:56]:<56} ║")
              lines.append(f"  ╠{'─' * 60}╣")
          sections = script.get("sections", [])
    for s in sections:
              name = f"[{s.get('timestamp','')}] {s.get('name','')}"
              lines.append(f"  ║  {name[:57]:<57} ║")
          lines.append(f"  ╠{'─' * 60}╣")
    runtime = script.get("estimated_runtime_mins", "?")
    tags = ", ".join(script.get("seo_tags", [])[:4])
    lines.append(f"  ║ ⏱  ~{runtime} mins  |  Tags: {tags[:38]:<38} ║")
    lines.append(f"  ╚{'═' * 60}╝\n")
    return "\n".join(lines)
